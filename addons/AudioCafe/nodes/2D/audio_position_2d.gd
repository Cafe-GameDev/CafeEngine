...
@export_group("AudioPosition Settings")
@export var state_sfx_map: Dictionary = {
    "default": "sfx_default_key"
}
...
func set_state(state_key: String):
    if not _audio_manifest:
        printerr("AudioManifest not loaded in AudioPosition2D.")
        return

    if not state_sfx_map.has(state_key):
        printerr("State key '%s' not found in state_sfx_map." % state_key)
        return

    var sfx_key = state_sfx_map[state_key]
